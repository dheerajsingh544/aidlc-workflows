...
2. [ ] **Log questions via `aidlc-log.ts`** — before presenting a structured question: `bun .codex/tools/aidlc-log.ts decision --stage <slug> --decision "<summary>" --options "<csv>"`. After response: `bun .codex/tools/aidlc-log.ts answer --stage <slug> --details "<exact choice>"`. (§3)
...
1. Optional before the human prompt: `bun .codex/tools/aidlc-state.ts gate-start <slug>` — marks the stage `[-]` → `[?]` and emits `STAGE_AWAITING_APPROVAL`. The stage is now on-hold waiting for the user; `/aidlc --status` will show "Awaiting your approval on <stage-name>". If this step is missed, the later `report --stage <slug> --result approved` opens the missing gate before approval, and `reject <slug>` likewise backfills it before the rejection (both backfilled rows carry `Recovered: true`).
...
3. Based on the user response:
   ...
   - **Request Changes** → `bun .codex/tools/aidlc-state.ts reject <slug> --feedback "<text>"`. The tool emits `GATE_REJECTED` + `STAGE_REVISING`, marks `[?]` → `[R]`, increments Revision Count. If gate-start was skipped (stage still `[-]`), reject backfills the missing `STAGE_AWAITING_APPROVAL` first — mirroring the approve-side backfill. After re-running the stage work, call `bun .codex/tools/aidlc-state.ts revise <slug>` to re-enter the gate (emits a fresh `STAGE_AWAITING_APPROVAL`, marks `[R]` → `[?]`).
...
1. Advance state atomically via CLI tool (see "Silent bookkeeping writes" below):
   `bun .codex/tools/aidlc-state.ts advance "<completed-slug>" "<next-slug>"`
...
1. BEFORE presenting the approval question: optionally `bun .codex/tools/aidlc-state.ts gate-start <slug>` (emits `STAGE_AWAITING_APPROVAL` and makes status truthful while the prompt is open).
2. AFTER user response: `bun .codex/tools/aidlc-orchestrate.ts report --stage <slug> --result approved --user-input "<choice>"` or `bun .codex/tools/aidlc-state.ts reject <slug> --feedback "<text>"`. `report` emits any missing gate row, then `GATE_APPROVED` + `STAGE_COMPLETED`, and auto-advances to the next in-scope stage (or completes the workflow if this was the final stage). `reject` likewise emits any missing gate row, then `GATE_REJECTED` + `STAGE_REVISING`, and leaves the stage in `[R]`.

At each question interaction:
1. BEFORE presenting the question: `bun .codex/tools/aidlc-log.ts decision --stage <slug> --decision "<summary>" --options "<A,B,C>"` (emits `DECISION_RECORDED`).
2. AFTER response: `bun .codex/tools/aidlc-log.ts answer --stage <slug> --details "<summary of answers>"` (emits `QUESTION_ANSWERED`).
...
**Enforcement:** State file updates happen automatically via the PostToolUse hook when `TaskUpdate` sets a stage task to `in_progress` with a `[slug]` suffix in `activeForm`. At stage END, `bun .codex/tools/aidlc-orchestrate.ts report --stage <slug> --result approved` marks the completed stage `[x]`, auto-advances to the next in-scope stage, and handles completion bookkeeping. Do not skip the intermediate `[-]` state by going directly from `[ ]` to `[x]`.
...
# Mark stage complete
bun .codex/tools/aidlc-state.ts checkbox "SLUG=completed"
```

**Field updates** (aidlc-state.md) — the tool writes fields in `- **Field Name**: value` format:
```bash
bun .codex/tools/aidlc-state.ts set "Current Stage=STAGE_NAME" "Lifecycle Phase=PHASE" "Status=In Progress" "Last Updated=NOW" "Active Agent=AGENT_NAME" "In Progress=STAGE_NAME"
```
...
```bash
bun .codex/tools/aidlc-state.ts finalize "completed-slug"
```
...
```bash
bun .codex/tools/aidlc-state.ts complete-workflow "completed-slug"
```
...
```bash
bun .codex/tools/aidlc-state.ts lookup phase-of SLUG          # → phase name
bun .codex/tools/aidlc-state.ts lookup next-stage SLUG SCOPE   # → next in-scope slug
bun .codex/tools/aidlc-state.ts lookup agent-for SLUG          # → lead agent name
bun .codex/tools/aidlc-state.ts lookup validate-stage SLUG     # → JSON with slug, phase, number, valid
```
...
1. `.codex/aidlc-rules/` — organization and project guardrails (always)
...
| **Scope** | Controls which stages execute and at what depth. Nine built-in scopes, one file per scope under `.codex/scopes/aidlc-<name>.md`: enterprise, feature, mvp, poc, bugfix, refactor, infra, security-patch, workshop. Custom scopes can be added without editing this file. |
...
| **Guardrail** | A learned behavioral rule (org-level or project-level) stored in `.codex/aidlc-rules/` |
...
2. **Surface candidates (the tool reads memory.md).** Run:
   ```bash
   bun .codex/tools/aidlc-learnings.ts surface --slug <stage-slug>
   ```
...
5. **Persist (the tool writes + emits audit).** Build the selections file and call:
   ```bash
   bun .codex/tools/aidlc-learnings.ts persist --slug <stage-slug> --selections-json <path>
   ```
